...
- `tests/check_refactor.py`：使用重构前夹具冻结结构边界，并以当前黄金摘要执行主机 C 行为回归。
- `firmware_releases`：按标签归档的 HEX 文件；文件名含 `unverified` 的快照尚未完成实机验收。

## 版本路线

当前工作区已完成全工程 24 个模块的保守精简，并将主通信接口收敛到 CN6/USART2；三轴运动、灯板串口控制和 MCP4728 易失寄存器写入功能保持不变。当前代码已通过主机黄金回归与 Keil 构建，但 Y/Z 运动、限位和 MCP4728 仍未完成完整实机验收；本轮归档为 `dev-20260904-code-cleanup-baseline-unverified` 开发基线，不作为实机通过版本。验证结果和上板清单见 [重构测试方案](doc/refactor_test_plan.md)。
...
| `dev-20260904-code-cleanup-baseline-unverified` | 完成 24 个模块的代码精简，删除无用调试状态、旧自检、未启用 USART3 和冗余 HAL 配置；同步当前结构冻结与 52 组主机黄金回归。 | Keil AC6.24 全量构建 0 错误、0 警告；结构、三轴专项及 52 组主机回归通过。主通信为 CN6/USART2；未新增实机验收，Y/Z、其余限位、MCP4728 及实时性限制沿用上一开发快照，因此仅作为后续开发基线。 |
...
当前主机通信使用 CN6/USART2（PA2/PA3）；CN4/USART3（PC10/PC11）不启用。运动帧轴号 `01/02/03` 分别选择 X/Y/Z；每条命令只运行一轴，不做多轴同步插补，也不允许另一轴在当前运动完成前启动。`0x0200` 请求帧的速度单位为 STEP/s、距离单位为 STEP 脉冲数，正常完成后回传 `0x0201` 并保持该轴驱动使能；`0x0202` 只停止帧中指定的当前活动轴，`0x0203` 查询当前状态。验证用 `0x02F0` 为单轴持续运行命令。
