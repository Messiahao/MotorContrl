...
- `MotorContrl/APPs`：固定轮询、串口、运动、辅助输出、限位、LED、自检与灯板占位。
...
当前工作区已做分层提取，灯板仅为空接口；本次尚未完成实机验收，不属于下面的已归档发布版本。验证结果和上板清单见 [重构测试方案](doc/refactor_test_plan.md)。
...
- 限位保护代码已保存于开发快照，但尚未进行实机限位触发验证；限位开关到货前不得据此进行机构运动。
...
STEP/s、距离单位为 STEP 脉冲数，正常完成后回传 `0x0201`；`0x0202` 为主机停止命令，停止后回传
`0x0202`；`0x0203` 查询当前状态。当前未归档工作区另有验证用 `0x02F0` 持续运行命令，运行中
轮询 X 轴 PC6/PB15/PB14 三路低有效限位，触发后停止并回传错误码 `0x0B`（`0x02FF`）。该保护
...
本次归档后的下一项工作是光电限位开关逐路实机测试；在该测试完成前，不把限位保护标记为已验收。MCP4728/I2C 仍保持空接口，CN10 四路输出尚未纳入本版本的已实现功能。
