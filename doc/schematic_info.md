...
- 当前固件已将九路限位配置为双边沿 EXTI，并启用 EXTI9_5、EXTI15_10 两组 NVIC；上升沿表示遮光/故障有效。X 轴的 PC6/PB15/PB14 上升沿会立即停止 X 轴 STEP 但保持 X 驱动使能，Y/Z 限位只更新各自状态，待对应轴运动路径实现后再接入停机。后续若增加 ENN 外部上拉，软件仍保持低有效逻辑。
